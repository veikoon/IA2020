...
from pacman.api import GameState
...
ERROR_HINT_MAP = {
  'ex08': {
    "<type 'exceptions.IndexError'>": """
      We noticed that your project threw an IndexError on ex08.
...
def getDisplay(graphicsByDefault, options=None):
    ...
    if graphics:
        try:
            import pacman.graphicsDisplay
            return pacman.graphicsDisplay.PacmanGraphics(1, frameTime=.05)
        except ImportError:
            pass
    import pacman.textDisplay
    return pacman.textDisplay.NullGraphics()
...
if __name__ == '__main__':
    ...
    # pacman modules 
    for cp in ['agents.py', 'problems.py', 'heuristics.py']:
        moduleName = re.match('.*?([^/]*)\.py', cp).group(1)
        moduleDict[moduleName] = loadModuleFile(moduleName, os.path.join(os.getcwd(), options.codeRoot, 'pacman', cp))
