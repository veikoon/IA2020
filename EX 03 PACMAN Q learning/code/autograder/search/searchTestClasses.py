...
import pacman.api as api
from pacman.problems import SearchProblem
...
class GraphSearchTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        for module in moduleDict.values():
            if hasattr(module, self.alg):
                search = module
                break
        searchAgents = moduleDict['agents']
...
class PacmanSearchTest(testClasses.TestCase):
    ...
    def getSolInfo(self, search, searchAgents, searchProblems, heuristics):
        alg = getattr(search, self.alg)
        lay = layout.Layout([l.strip() for l in self.layout_text.split('\n')])
        start_state = api.GameState()
        ...
        heuristic = getattr(heuristics, self.heuristicName) if self.heuristicName != None else None
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for module in moduleDict.values():
            if hasattr(module, self.alg):
                ...
                break
        searchAgents = moduleDict['agents']
        searchProblems = moduleDict['problems']
        heuristics = moduleDict['heuristics']
        gold_solution = [str.split(solutionDict['solution']), str.split(solutionDict['rev_solution'])]
        gold_expanded = max(int(solutionDict['expanded_nodes']), int(solutionDict['rev_expanded_nodes']))

        solution, expanded, error = self.getSolInfo(search, searchAgents, searchProblems, heuristics)
        if error != None:
            grades.addMessage('FAIL: %s' % self.path)
            grades.addMessage('%s' % error)
            return False
...
class CornerProblemTest(testClasses.TestCase):
    ...
    def solution(self, search, searchAgents):
        ...
        problem = searchAgents.CornersSearchProblem(gameState)
    ...
    def execute(self, grades, moduleDict, solutionDict):
        for module in moduleDict.values():
            if hasattr(module, 'breadth_first_search'):
                search = module
                break
        for module in moduleDict.values():
            if hasattr(module, 'CornersSearchProblem'):
                searchAgents = module
                break
...
class HeuristicTest(testClasses.TestCase):
    ...
    def setupProblem(self, searchAgents, searchHeuristics):
        lay = layout.Layout([l.strip() for l in self.layoutText.split('\n')])
        gameState = api.GameState()
        gameState.initialize(lay, 0)
    ...
    def execute(self, grades, moduleDict, solutionDict):
        for module in moduleDict.values():
            if hasattr(module, 'astar_search'):
                search = module
                break
        for module in moduleDict.values():
            if hasattr(module, self.searchProblemClassName):
                searchAgents = module
                break
        for module in moduleDict.values():
            if hasattr(module, self.heuristicName):
                searchHeuristics = module
                break
        solutionCost = int(solutionDict['solution_cost'])
        problem, state, heuristic = self.setupProblem(searchAgents, searchHeuristics)
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for module in moduleDict.values():
            if hasattr(module, self.heuristicName):
                searchHeuristics = module
                break
        problem, _, heuristic = self.setupProblem(searchAgents, searchHeuristics)

        path = search.astar_search(problem, heuristic)
...
class ClosestDotTest(testClasses.TestCase):
    ...
    def solution(self, search, searchAgents, searchProblems, searchCosts):
        lay = layout.Layout([l.strip() for l in self.layoutText.split('\n')])
        gameState = api.GameState()
        gameState.initialize(lay, 0)
        agent = searchAgents.ClosestDotSearchAgent(search.breadth_first_search, searchProblems.AnyFoodSearchProblem, costFn=searchCosts.all_equal_cost)
        path = agent.searchFunction(agent.searchType(gameState))
        return path
    ...
    def execute(self, grades, moduleDict, solutionDict):
        for module in moduleDict.values():
            if hasattr(module, 'breadth_first_search'):
                ...
                break
        for module in moduleDict.values():
            if hasattr(module, 'ClosestDotSearchAgent'):
                searchAgents = module
                break
        for module in moduleDict.values():
            if hasattr(module, 'AnyFoodSearchProblem'):
                searchProblems = module
                break
        searchCosts = moduleDict['heuristics']

        gold_length = int(solutionDict['solution_length'])
        solution = self.solution(search, searchAgents, searchProblems, searchCosts)
...
class CornerHeuristicSanity(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for module in moduleDict.values():
            if hasattr(module, 'CornersSearchProblem'):
                ...
                break
        for module in moduleDict.values():
            if hasattr(module, 'corners_heuristic'):
                corners_heuristic = module.corners_heuristic
                break
        game_state = api.GameState()
        lay = layout.Layout([l.strip() for l in self.layout_text.split('\n')])
        game_state.initialize(lay, 0)
        problem = searchAgents.CornersSearchProblem(game_state)
        start_state = problem.getStartState()
        h0 = corners_heuristic(start_state, problem)
        succs = problem.getSuccessors(start_state)
        # cornerConsistencyA
        for succ in succs:
            h1 = corners_heuristic(succ[0], problem)
            if h0 - h1 > 1:
                grades.addMessage('FAIL: inconsistent heuristic')
                return False
        heuristic_cost = corners_heuristic(start_state, problem)
        ...
        heuristics = []
        for state in states:
            heuristics.append(corners_heuristic(state, problem))
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        start_state = api.GameState()
        start_state.initialize(lay, 0)
        problem = searchAgents.CornersSearchProblem(start_state)
        solution = search.astar_search(problem, corners_heuristic)
        handle.write('cost: "%d"\n' % len(solution))
        handle.write('path: """\n%s\n"""\n' % wrap_solution(solution))
        handle.close()
        return True
...
class CornerHeuristicPacman(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        problem = searchAgents.CornersSearchProblem(game_state)
        start_state = problem.getStartState()
        if corners_heuristic(start_state, problem) > true_cost:
            grades.addMessage('FAIL: Inadmissible heuristic')
            return False
        path = search.astar_search(problem, corners_heuristic)
        print("path:", path)
