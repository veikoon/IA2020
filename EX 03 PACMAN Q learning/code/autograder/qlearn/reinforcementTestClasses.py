...
import pacman.layout, pacman.textDisplay, pacman
import gridworld.api as gridworld
import time
...
class ValueIterationTest(testClasses.TestCase):
    ...
    def runAgent(self, moduleDict, numIterations):
        for module in moduleDict.values():
            if hasattr(module, 'value_iteration'):
                value_iteration = module.value_iteration
                break
        agent = moduleDict['learning'].ValueIterationAgent(value_iteration, self.grid, discount=self.discount, iterations=numIterations)
...
class AsynchronousValueIterationTest(ValueIterationTest):
    def runAgent(self, moduleDict, numIterations):
        for module in moduleDict.values():
            if hasattr(module, 'async_value_iteration'):
                async_value_iteration = module.async_value_iteration
                break
        agent = moduleDict['learning'].ValueIterationAgent(async_value_iteration, self.grid, discount=self.discount, iterations=numIterations)
        states = self.grid.getStates()
        actions = list(reduce(lambda a, b: set(a).union(b), [self.grid.getPossibleActions(state) for state in states]))
        values = {}
        qValues = {}
...
class ApproximateQLearningTest(testClasses.TestCase):
    ...
    def runAgent(self, moduleDict, numExperiences):
        for module in moduleDict.values():
            if hasattr(module, 'ApproximateQAgent'):
                ApproximateQAgent = module.ApproximateQAgent
                break
        for module in moduleDict.values():
            if hasattr(module, self.extractor):
                extractor = getattr(module, self.extractor)
                break
        agent = ApproximateQAgent(extractor=extractor,**self.opts)
...
class QLearningTest(testClasses.TestCase):
    ...
    def runAgent(self, moduleDict, numExperiences):
        for module in moduleDict.values():
            if hasattr(module, 'QLearningAgent'):
                qlearn = module
                break
        agent = qlearn.QLearningAgent(**self.opts)
        states = [state for state in self.grid.getStates() if len(self.grid.getPossibleActions(state)) > 0]
        states.sort()
        lastExperience = None
        for i in range(numExperiences):
...
class EpsilonGreedyTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        if sys.platform == 'win32':
            ...
            _, question_name, test_name = testDict['test_out_file'].split('/')
        self.experiences = Experiences(test_name.split('.')[0][:-4]) # strip the trailing "-eps" in the name
...
def computePolicy(moduleDict, grid, discount):
    for module in moduleDict.values():
        if hasattr(module, 'value_iteration'):
            value_iteration = module.value_iteration
            break
    agent = moduleDict['learning'].ValueIterationAgent(value_iteration, grid, discount=discount)
    policy = {}
...
class GridPolicyTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        for module in moduleDict.values():
            if hasattr(module, 'mdp_parameters'):
                mdp_parameters = module.mdp_parameters
                break

        result = mdp_parameters(self.parameterFn)
