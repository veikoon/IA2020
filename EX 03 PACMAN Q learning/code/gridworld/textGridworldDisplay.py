...
from pacman.util import Counter
...
if __name__ == '__main__':
    ...
    values = Counter(dict([(state,1000.23) for state in grid.getStates()]))
    ...
    qValues = Counter(dict([((state, action), 10.5) for state, action in qStates]))
    qValues = Counter(dict([((state, action), 10.5) for state, action in reduce(lambda x,y: x+y, stateCrossActions, [])]))
